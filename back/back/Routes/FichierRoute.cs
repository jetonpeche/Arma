...
public static class FichierRoute
{
     ...
     )
     {
          ...
          switch (_requete.TypeRessource)
          {
               ...
               case ETypeRessource.Orbat:
                    var nouveauNomFichierOrbat = await UploadAsync<Orbat>(
                          _requete.idRessource,
                          _requete.Fichier,
                          Constant.CHEMIN_IMG_ORBAT,
                          (x) => x.NomImage,
                          (x, s) => x.NomImage = s
                    );

                    return Results.Ok(ConstruireUrlFichier(_httpContext, Constant.CHEMIN_IMG_ORBAT + nouveauNomFichierOrbat));
          }
